...
if __name__ == '__main__':
    ...
    print("apply transforms one-by-ine took %fs for each image" % ((time.time() - st) / 100))
    ...
    def fast_affine_transfrom(image):
        ...
        M_combined = M_rotate.dot(M_shift).dot(M_flip).dot(M_zoom).dot(
            M_shear
        )  # this means rotation is the first transformation
    ...
    ## C. in practice, we use TF dataset API to load data for training
    n_data = 100
    ...
    def generator():
        ...
        for _input, _target in zip(imgs_file_list, train_targets):
            # yield _input.encode('utf-8'), _target.encode('utf-8')#cPickle.dumps(_target)
