...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
        ...
        train_op = tf.train.AdamOptimizer(LEARNING_RATE).minimize(loss, var_list=stylied_dec_net.all_weights)
        ...
        for epoch in range(EPOCHS):
            ...
            for batch in range(n_batches):
                ...
                if step % 1000 == 0:
                    ...
                    tl.files.save_npz(stylied_dec_net.all_weights, name=MODEL_SAVE_PATH + str(step) + '_model.npz')
