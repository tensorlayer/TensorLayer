...
def main(_):
    ...
    def inference(x, is_training, num_steps, reuse=None):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            network = tl.layers.RNNLayer(
                ...
                cell_init_args={'forget_bias':
                                0.0},  # 'state_is_tuple': True},
    ...
    network, lstm1, lstm2 = inference(
        input_data, is_training=True, num_steps=num_steps, reuse=None)
    # Inference for Validating
    network_val, lstm1_val, lstm2_val = inference(
        input_data, is_training=False, num_steps=num_steps, reuse=True)
    # Inference for Testing (Evaluation)
    network_test, lstm1_test, lstm2_test = inference(
        input_data_test, is_training=False, num_steps=1, reuse=True)
    ...
    for i in range(max_max_epoch):
        ...
        # Training
        print("Epoch: %d/%d Learning rate: %.3f" % (i + 1, max_max_epoch,
                                                    sess.run(lr)))
        ...
        for step, (x, y) in enumerate(
            ...
            _cost, state1, state2, _ = sess.run(
                [cost, lstm1.final_state, lstm2.final_state, train_op],
                feed_dict=feed_dict)
            costs += _cost
            iters += num_steps
    ...
    for step, (x, y) in enumerate(
        ...
        _cost, state1, state2 = sess.run(
            [cost_test, lstm1_test.final_state, lstm2_test.final_state],
            feed_dict=feed_dict)
        costs += _cost
        iters += 1
    test_perplexity = np.exp(costs / iters)
    print("Test Perplexity: %.3f took %.2fs" % (test_perplexity,
