...
def _data_aug_fn(im, ann):
    ...
    im, clas, coords = tl.prepro.obj_box_crop(im, clas, coords, \
        wrg=im_size[1], hrg=im_size[0], is_rescale=True, is_center=True, is_random=True)
...
for i in range(len(im)):
    tl.vis.draw_boxes_and_labels_to_image(im[i] * 255, ann[i][0], ann[i][1], \
        [], classes, True, save_name='_bbox_vis_%d.png' % i)
