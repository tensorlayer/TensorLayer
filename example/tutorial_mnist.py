...
def main_test_layers(model='relu'):
    ...
    if model == 'relu':
        ...
        network = tl.layers.DropconnectDenseLayer(
            network, keep=0.5, n_units=10, act=tf.identity, name='output')
    ...
    for epoch in range(n_epoch):
        ...
        for X_train_a, y_train_a in tl.iterate.minibatches(
            ...
            feed_dict = {x: X_train_a, y_: y_train_a}
            feed_dict.update(
                network.all_drop)  # enable dropout or dropconnect layers
        ...
        if epoch + 1 == 1 or (epoch + 1) % print_freq == 0:
            ...
            try:
                ...
                print(
                    "You should change vis.W(), if you want to save the feature images for different dataset"
                )
...
def main_test_denoise_AE(model='relu'):
    ...
    if model == 'relu':
        ...
        network = tl.layers.DropoutLayer(
            network, keep=0.5,
            name='denoising1')  # if drop some inputs, it is denoise AE
        network = tl.layers.DenseLayer(
            network, n_units=196, act=tf.nn.relu, name='relu1')
        recon_layer1 = tl.layers.ReconLayer(
            network,
            x_recon=x,
            n_units=784,
            act=tf.nn.softplus,
        ...
        network = tl.layers.DenseLayer(
            network, n_units=196, act=tf.nn.sigmoid, name='sigmoid1')
        recon_layer1 = tl.layers.ReconLayer(
            network,
            x_recon=x,
            n_units=784,
...
def main_test_stacked_denoise_AE(model='relu'):
    ...
    network = tl.layers.DropoutLayer(network, keep=0.8, name='drop1')
    network = tl.layers.DenseLayer(
        network, n_units=800, act=act, name=model + '1')
    x_recon1 = network.outputs
    recon_layer1 = tl.layers.ReconLayer(
        network, x_recon=x, n_units=784, act=act_recon, name='recon_layer1')
    # 2nd layer
    network = tl.layers.DropoutLayer(network, keep=0.5, name='drop2')
    network = tl.layers.DenseLayer(
        network, n_units=800, act=act, name=model + '2')
    recon_layer2 = tl.layers.ReconLayer(
        network,
        x_recon=x_recon1,
        n_units=800,
        act=act_recon,
...
def main_test_cnn_layer():
    ...
    x = tf.placeholder(
        tf.float32, shape=[batch_size, 28, 28,
                           1])  # [batch_size, height, width, channels]
    y_ = tf.placeholder(
        tf.int64, shape=[
            batch_size,
        ])
    ...
    network = tl.layers.MaxPool2d(
        network, (2, 2), (2, 2), padding='SAME', name='pool1')
    network = tl.layers.Conv2d(
        network,
        64, (5, 5), (1, 1),
        act=tf.nn.relu,
        ...
        name='cnn2')
    network = tl.layers.MaxPool2d(
        network, (2, 2), (2, 2), padding='SAME', name='pool2')
    ## end of conv
        ...
        if epoch + 1 == 1 or (epoch + 1) % print_freq == 0:
            ...
            try:
                tl.vis.CNN2d(
                    ...
                    second=10,
                    saveable=True,
                    name='cnn1_' + str(epoch + 1),
                    fig_idx=2012)
            except:
                print(
                    "You should change vis.CNN(), if you want to save the feature images for different dataset"
                )
