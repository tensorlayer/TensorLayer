...
for idx in range(len(caption_data['images'])):
    ...
    processed_capts.append(
        tl.nlp.process_sentence(img_capt, start_word="<S>", end_word="</S>"))
print("Original Captions: %s" % img_capts)
print("Processed Captions: %s\n" % processed_capts)
# build vocab
_ = tl.nlp.create_vocab(
    processed_capts, word_counts_output_file=VOC_FIR, min_word_count=1)
vocab = tl.nlp.Vocabulary(
    VOC_FIR, start_word="<S>", end_word="</S>", unk_word="<UNK>")
...
tl.visualize.frame(
    np.asarray(im), second=1, saveable=False, name='frame', fig_idx=1236)
...
input_queue = prefetch_input_data(
    reader,
    file_pattern=
    "train.cat_caption",  # sets train.???_caption to read many files
    ...
    2,  # minimum number of shards to keep in the input queue.
    num_reader_threads=
...
img_batch, img_cap_batch, img_cap_ids_batch = tf.train.batch(
    [img, img_cap, img_cap_ids
     ],  # Note: shuffle_batch doesn't support dynamic_pad
    batch_size=4,
    capacity=50000,
...
for _ in range(3):
    ...
    imgs, caps, caps_id = sess.run(
        [img_batch, img_cap_batch,
         img_cap_ids_batch])  # batch of examples with dynamic_pad
    print(caps)
    print(caps_id)
    tl.visualize.images2d(
        (imgs + 1) / 2,
        second=1,
        saveable=False,
        name='batch',
...
for _ in range(3):
    print("Step %s" % _)
    imgs, inputs, targets, masks = sess.run(
