...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
        ...
        encoder.restore_model(sess,content_enc_net,ENCODER_PATH)
        encoder.restore_model(sess,style_enc_net,ENCODER_PATH)
        encoder.restore_model(sess,stylied_enc_net,ENCODER_PATH)
        ...
        for epoch in range(EPOCHS):
            ...
            for batch in range(n_batches):
                ...
                if step % 1000 == 0:
                    ...
                    result_image = sess.run(stylied_image,feed_dict={content_input: content_batch, style_input: style_batch})
