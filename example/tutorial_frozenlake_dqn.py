...
loss = tl.cost.mean_squared_error(
    nextQ, y, is_mean=False)  # tf.reduce_sum(tf.square(nextQ - y))
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        print("Episode [%d/%d] sum reward:%f running reward:%f took:%.5fs %s" %
              (i, num_episodes, rAll, running_reward,
               time.time() - episode_time, '' if rAll == 0 else ' !!!!!!!!'))
