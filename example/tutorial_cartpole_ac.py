...
print("observation high: %s" %
      env.observation_space.high)  # [ 2.4 , inf , 0.41887902 , inf]
print("observation low : %s" %
      env.observation_space.low)  # [-2.4 , -inf , -0.41887902 , -inf]
...
class Actor(object):
    ...
    def choose_action(self, s):
        probs = self.sess.run(self.acts_prob, {
            self.s: [s]
        })  # get probabilities of all actions
    ...
    while True:
        ...
        if done or t >= MAX_EP_STEPS:
            ...
            print("Episode: %d reward: %f running_reward %f took: %.5f" %
                  (i_episode, ep_rs_sum, running_reward,
            ...
            if t >= MAX_EP_STEPS:
                ...
                while True:
                    env.render()
                    # a = actor.choose_action(s)
                    a = actor.choose_action_greedy(
                        s)  # Hao Dong: it is important for this task
                    s_new, r, done, info = env.step(a)
