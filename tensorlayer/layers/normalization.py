    ...
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            if tf.__version__ > '0.12.1' and beta_init == tf.zeros_initializer:
                beta_init = beta_init()
            beta = tf.get_variable('beta', shape=params_shape, initializer=beta_init, dtype=LayersConfig.D_TYPE, trainable=is_train)
            ...
            if tf.__version__ > '0.12.1':
                ...
                moving_mean_init = tf.zeros_initializer
            moving_mean = tf.get_variable('moving_mean', params_shape, initializer=moving_mean_init, dtype=LayersConfig.D_TYPE, trainable=False)
    ...
    def __init__(
        ...
        with tf.variable_scope(name) as vs:
            ...
            scale = tf.get_variable(
                'scale', [self.inputs.get_shape()[-1]], initializer=tf.truncated_normal_initializer(mean=1.0, stddev=0.02), dtype=LayersConfig.D_TYPE)
            offset = tf.get_variable('offset', [self.inputs.get_shape()[-1]], initializer=tf.constant_initializer(0.0), dtype=LayersConfig.D_TYPE)
