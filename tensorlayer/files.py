...
def load_fashion_mnist_dataset(shape=(-1, 784), path="data"):
    """Load fashion-MNIST dataset.(https://github.com/zalandoresearch/fashion-mnist)
    ...
    # For convenience, they also download the requested files if needed.
    def load_mnist_images(path, filename):
        filepath = maybe_download_and_extract(filename, path, 'http://fashion-mnist.s3-website.eu-central-1.amazonaws.com/')
    ...
    # Download and read the training and test set images and labels.
    logging.info("Load or Download fashion-MNIST > {}".format(path))
    X_train = load_mnist_images(path, 'train-images-idx3-ubyte.gz')
    y_train = load_mnist_labels(path, 'train-labels-idx1-ubyte.gz')
    X_test = load_mnist_images(path, 't10k-images-idx3-ubyte.gz')
    y_test = load_mnist_labels(path, 't10k-labels-idx1-ubyte.gz')
