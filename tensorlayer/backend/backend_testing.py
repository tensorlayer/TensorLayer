...
def get_variable_with_initializer(scope_name, var_name, shape, init=tl.initializers.random_normal()):
    ...
    var = tl.Variable(initial_value=initial_value, name=var_name)
    return var
...
def Input(shape, dtype=tf.float32, name=None):
    ...
    >>> print(x)
    """
    x = tl.layers.Input(shape, dtype, name)
    return x
...
def activation(x, act='relu'):
    ...
    Examples
    ----------
    >>> x = tl.constant([-float("inf"), -5, -0.5, 1, 1.2, 2, 3, float("inf")])
    >>> print(activation(x,'relu'))
    """
    _act_dict = {
        "relu": tl.nn.relu,
        "relu6": tl.nn.relu6,
        "leaky_relu": tl.nn.leaky_relu,
        "lrelu": tl.nn.leaky_relu,
        "softplus": tl.nn.softplus,
        "tanh": tl.nn.tanh,
        "sigmoid": tl.nn.sigmoid,
    }
...
def conv2d(x, n_filter, filter_size, strides=(1, 1), padding='same',
    ...
    if b_init is not None:
        ...
        x = tl.nn.bias_add(x, b_init, data_format=data_format,)
    if act is not None:
        x = activation(x, act)
    return x
...
def pool2d(x, filter_size, strides=(1, 1),
    ...
    # data_format, padding = preprocess_format(data_format, padding)
    if pool_mode == 'max':
        x = tl.nn.max_pool(x, filter_size, strides,
                           padding=padding,
                           name=name)
    elif pool_mode == 'avg':
        x = tl.nn.avg_pool(x, filter_size, strides,
                           padding=padding,
                           name=name)
    else:
...
if __name__ == '__main__':
    ...
    conv2d_2 = conv2d(conv2d_1,n_filter=64, filter_size=(3, 3), strides=(2, 2), b_init=b_init, name='conv2d_2',act='softplus',data_format='NHWC')
    print(tf.float32, tf.DType)
    #
    # # 'activation'
    # x = tl.constant([-float("inf"), -5, -0.5, 1, 1.2, 2, 3, float("inf")])
    # print(activation(x,'lrelu'))
    #
    # # pooling
    # net = tl.layers.Input([3, 50, 50, 32], name='input')
    # net = pool2d(net, filter_size=(2, 2), strides=(2, 2), padding='same', pool_mode='max')
    # print(net)
    #
    # # flatten
    # net = tl.layers.Input([3, 50, 50, 32], name='input')
