...
def _add_deprecated_function_notice_to_docstring(doc, date, instructions):
    ...
    return decorator_utils.add_notice_to_docstring(doc, instructions, 'DEPRECATED FUNCTION', '(deprecated)', main_text)
...
def _add_deprecated_arg_notice_to_docstring(doc, date, instructions):
    ...
    return decorator_utils.add_notice_to_docstring(
        doc, instructions, 'DEPRECATED FUNCTION ARGUMENTS', '(deprecated arguments)', [
            ...
            'They will be removed %s.' % ('in a future version'
                                          if date is None else ('after %s' % date)), 'Instructions for updating:'
        ]
    )
...
def deprecated_args(date, instructions, *deprecated_arg_names_or_tuples, **kwargs):
  ...
  """
    ...
    """
        arg_name_to_pos = {name: pos for pos, name in enumerate(arg_spec.args)}
    ...
    def deprecated_wrapper(func):
      ...
      """
        ...
        return tf_decorator.make_decorator(
            func, new_func, 'deprecated', _add_deprecated_arg_notice_to_docstring(func.__doc__, date, instructions)
        )
                    ...
                    if arg_name in named_args and named_args[arg_name] == arg_value:
                        if (func, arg_name) not in _PRINTED_WARNING:
                            ...
                            logging.warning(
                                ...
                                'will be removed %s.\nInstructions for updating:\n%s', _call_location(),
                                decorator_utils.get_qualified_name(func), func.__module__, arg_name, arg_value,
...
def rewrite_argument_docstring(old_doc, old_argument, new_argument):
    return old_doc.replace('`%s`' % old_argument,
                           '`%s`' % new_argument).replace('%s:' % old_argument, '%s:' % new_argument)
