...
def load_wmt_en_fr_dataset(path='data', name='wmt_en_fr'):
    ...
    def get_wmt_enfr_train_set(path):
        ...
        maybe_download_and_extract(filename, path, WMT_ENFR_TRAIN_URL, extract=True)
    ...
    def get_wmt_enfr_dev_set(path):
        """Download the WMT en-fr training corpus to directory unless it's there."""
        filename = "dev-v2.tgz"
        dev_file = maybe_download_and_extract(filename, path, WMT_ENFR_DEV_URL, extract=False)
