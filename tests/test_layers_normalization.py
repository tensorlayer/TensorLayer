...
def model(x, is_train=True, reuse=False):
    with tf.variable_scope("model", reuse=reuse):
        ...
        n = tl.layers.BatchNormLayer(n, is_train=is_train, name='norm_batch')(n)
        ...
        n.local_outputs = tf.reshape(n.local_outputs, [-1, 80, 100, 100])
        n = tl.layers.GroupNormLayer(groups=40, data_format='channels_first', name='groupnorm')(n)
        n.local_outputs = tf.reshape(n.local_outputs, [-1, 100, 100, 80])
        n = tl.layers.SwitchNormLayer(name='switchnorm')(n)
        n = tl.layers.QuantizedConv2dWithBN( n_filter=3, is_train=is_train, name='quan_cnn_with_bn')(n)
