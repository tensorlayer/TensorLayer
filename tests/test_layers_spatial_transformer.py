...
def model(x, is_train, reuse):
    with tf.variable_scope("STN", reuse=reuse):
        ...
        ## 3. Classifier
        n = tl.layers.Conv2d(
            n_filter=16, filter_size=(3, 3), strides=(2, 2), act=tf.nn.relu, padding='SAME', name='conv1'
        )(n)
...
class Layer_Spatial_Transformer_Test(CustomTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.net_weights = net.all_weights
        cls.net_n_weights = net.count_weights()
    ...
    def test_net_params(self):
        self.assertEqual(len(self.net_weights), 12)

    def test_net_n_params(self):
        self.assertEqual(self.net_n_weights, 1667980)
